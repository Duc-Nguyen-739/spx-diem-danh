...
- **Quét camera (2026-08-11)**: `camera-scan.html` = luồng decode (BarcodeDetector → jsQR → Quagga), `lib-jsqr.html` + `lib-quagga.html` = thư viện vendored (bọc `<script>`); `camera-css.html` = CSS overlay (nhúng sau `include('mobile')`); cả 4 nhúng qua scriptlet trước `include('js')`. Nút `#btnCamScan` + `#camFile` nằm ở `index.html` scan-row; wiring (change listener + disable nút) ở `js.html` (DOMContentLoaded + renderScanView). **GAS iframe chặn getUserMedia** (Permissions Policy) → trên GAS bấm nút = **mở TAB QUÉT RIÊNG** `?scan=1&tk=…` (window.open/anchor click — tab top-level camera chạy được, quét live kiểu AppSheet; iOS cần 1 chạm "▶ Bật camera" vì Apple bắt user gesture; kết quả gửi về qua postMessage + localStorage; popup bị chặn → fallback `<input capture>` camera native → auto decode multi-scale → Done). **Live loop iOS decode 800px/1 config yếu → AUTO-DECODE (2026-08-12)**: chạy ĐÚNG chain nút "📸 Chụp" (frame 1280px + jsQR + 3 config Quagga) theo nhịp cố định mỗi 2 tick frame (~1.2s) — KHÔNG gate theo độ rung (auto-snap cũ threshold 6/255 quá nhạy với noise iPhone → không bao giờ kích hoạt, bug 2026-08-12) → đưa mã vào khung là tự nhận Done, không cần bấm gì.
  **BUG TAB TRẮNG (2026-08-12, lặp lại nhiều lần trên Safari/Chrome)**: URL content GAS (`.../userCodeAppPanel?scan=1&tk=…`) KHÔNG serve thẳng app — nó là trang WRAPPER của GAS, nạp app vào `<iframe id=userHtmlFrame>` qua `document.write` (iframe src `/blank`) → `location.search` của app trong iframe có thể RỖNG (query bị nuốt) và `google.script` có thể chưa inject kịp khi js.html chạy → mock loader cũ `document.write` lúc document đã load xong = **XÓA TRẮNG toàn trang**. Fix: (1) js.html KHÔNG dùng document.write khi document không còn parse — thay bằng DOM injection (appendChild); (2) detect scan mode bằng CẢ `location.search` LẪN `document.referrer` (referrer của app trong tab quét = URL wrapper có scan=1&tk) → `window.__RC_SCAN_MODE__ = true` (camera-scan.html đặt trước js.html) → js.html skip mock + skip app init theo cờ; (3) `openCameraScan` GIỮ NGUYÊN query params của URL app khi thêm scan=1 (không split('?')[0]); (4) CSS scan-mode dùng `body.scan-mode` (không phải `.scan-mode body` — class trên body nên selector cũ không bao giờ khớp). Test: `tests/js-scanmode.test.js` (không document.write khi readyState≠loading) + `tests/camera-autosnap.test.js` (detect scan mode qua referrer). Ngoài GAS (preview/hosting top-level) = live scan video trong `#cameraModal`. Nút `#btnCamScan` **chỉ hiện trên mobile** (ẩn desktop qua `#btnCamScan{display:none}` ở camera-css.html, hiện lại trong media ≤900px ở camera-css.html — cả 2 rule CÙNG FILE, bug 2026-08-11 khi tách file). Thư viện ~130KB + ~156KB — dưới giới hạn file GAS 500KB nhưng quá 100KB nên không mở được trong UI editor (clasp push vẫn OK).
