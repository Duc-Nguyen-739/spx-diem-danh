...
const { inlineHtml, CSS_SCRIPTLET, MOBILE_SCRIPTLET, JS_SCRIPTLET } = require('../scripts/inline-html.js');
...
test('index.html chỉ chứa scriptlet — không khối <style>/<script> inline', () => {
  ...
  assert.ok(html.includes(MOBILE_SCRIPTLET), 'thiếu scriptlet mobile');
...
test('inline thay scriptlet bằng nội dung file', () => {
  const html = '<html><head>' + CSS_SCRIPTLET + MOBILE_SCRIPTLET + '</head><body>' + JS_SCRIPTLET + '</body></html>';
  const css = fs.readFileSync(CSS, 'utf8');
  const js = fs.readFileSync(JS, 'utf8');
  const mobile = fs.readFileSync(MOBILE, 'utf8');
  const out = inlineHtml(html, CSS, JS, MOBILE);
  ...
  assert.ok(!out.includes(CSS_SCRIPTLET) && !out.includes(MOBILE_SCRIPTLET) && !out.includes(JS_SCRIPTLET), 'còn sót scriptlet');
});

test('thiếu scriptlet → throw rõ lỗi (thứ tự check: css → js → mobile)', () => {
  assert.throws(() => inlineHtml('<html>no scriptlets</html>', CSS, JS, MOBILE), /css'/);
  assert.throws(() => inlineHtml('<html>' + CSS_SCRIPTLET + '</html>', CSS, JS, MOBILE), /js'/);
  assert.throws(() => inlineHtml('<html>' + CSS_SCRIPTLET + JS_SCRIPTLET + '</html>', CSS, JS, MOBILE), /mobile'/);
});

test('thiếu file css/mobile/js → throw', () => {
  const html = '<html>' + CSS_SCRIPTLET + MOBILE_SCRIPTLET + JS_SCRIPTLET + '</html>';
  assert.throws(() => inlineHtml(html, '/nonexistent/css.html', JS, MOBILE), /ENOENT/);
  assert.throws(() => inlineHtml(html, CSS, '/nonexistent/js.html', MOBILE), /ENOENT/);
  assert.throws(() => inlineHtml(html, CSS, JS, '/nonexistent/mobile.html'), /ENOENT/);
});
...
test('index.html THẬT: bản inline tự-chứa hoàn chỉnh', () => {
  ...
  // CSS + mobile + JS đầy đủ (wrapper + nội dung)
  ...
  assert.ok(!out.includes(CSS_SCRIPTLET) && !out.includes(MOBILE_SCRIPTLET) && !out.includes(JS_SCRIPTLET), 'còn scriptlet');
