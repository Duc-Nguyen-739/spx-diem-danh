...
const { inlineHtml, CSS_SCRIPTLET, MOBILE_SCRIPTLET, LIB_JSQR_SCRIPTLET, LIB_QUAGGA_SCRIPTLET, CAMERA_SCRIPTLET, JS_SCRIPTLET } = require('../scripts/inline-html.js');
...
test('index.html chỉ chứa scriptlet — không khối <style>/<script> inline', () => {
  ...
  for (const sc of [CSS_SCRIPTLET, MOBILE_SCRIPTLET, LIB_JSQR_SCRIPTLET, LIB_QUAGGA_SCRIPTLET, CAMERA_SCRIPTLET, JS_SCRIPTLET]) {
...
test('inline thay scriptlet bằng nội dung file (cả lib + camera)', () => {
  const html = '<html><head>' + CSS_SCRIPTLET + MOBILE_SCRIPTLET + '</head><body>' +
  ...
  const libQuagga = fs.readFileSync(LIB_QUAGGA, 'utf8');
  const camera = fs.readFileSync(CAMERA, 'utf8');
  const out = inlineHtml(html, CSS, JS, MOBILE, LIB_JSQR, LIB_QUAGGA, CAMERA);
  ...
  const leftover = [CSS_SCRIPTLET, MOBILE_SCRIPTLET, LIB_JSQR_SCRIPTLET, LIB_QUAGGA_SCRIPTLET, CAMERA_SCRIPTLET, JS_SCRIPTLET].filter((s) => out.includes(s));
  assert.deepStrictEqual(leftover, [], 'còn sót scriptlet: ' + leftover.join(', '));
});

test('thiếu scriptlet → throw rõ lỗi (thứ tự check: css → js → mobile → lib-jsqr → lib-quagga → camera)', () => {
  assert.throws(() => inlineHtml('<html>no scriptlets</html>', CSS, JS, MOBILE, LIB_JSQR, LIB_QUAGGA, CAMERA), /css'/);
  assert.throws(() => inlineHtml('<html>' + CSS_SCRIPTLET + '</html>', CSS, JS, MOBILE, LIB_JSQR, LIB_QUAGGA, CAMERA), /js'/);
  assert.throws(() => inlineHtml('<html>' + CSS_SCRIPTLET + JS_SCRIPTLET + '</html>', CSS, JS, MOBILE, LIB_JSQR, LIB_QUAGGA, CAMERA), /mobile'/);
  assert.throws(() => inlineHtml('<html>' + CSS_SCRIPTLET + MOBILE_SCRIPTLET + JS_SCRIPTLET + '</html>', CSS, JS, MOBILE, LIB_JSQR, LIB_QUAGGA, CAMERA), /lib-jsqr'/);
  assert.throws(() => inlineHtml('<html>' + CSS_SCRIPTLET + MOBILE_SCRIPTLET + LIB_JSQR_SCRIPTLET + JS_SCRIPTLET + '</html>', CSS, JS, MOBILE, LIB_JSQR, LIB_QUAGGA, CAMERA), /lib-quagga'/);
  assert.throws(() => inlineHtml('<html>' + CSS_SCRIPTLET + MOBILE_SCRIPTLET + LIB_JSQR_SCRIPTLET + LIB_QUAGGA_SCRIPTLET + JS_SCRIPTLET + '</html>', CSS, JS, MOBILE, LIB_JSQR, LIB_QUAGGA, CAMERA), /camera-scan'/);
});

test('thiếu file (css/mobile/js/lib/camera) → throw', () => {
  const all = CSS_SCRIPTLET + MOBILE_SCRIPTLET + LIB_JSQR_SCRIPTLET + LIB_QUAGGA_SCRIPTLET + CAMERA_SCRIPTLET + JS_SCRIPTLET;
  const html = '<html>' + all + '</html>';
  assert.throws(() => inlineHtml(html, '/nonexistent/css.html', JS, MOBILE, LIB_JSQR, LIB_QUAGGA, CAMERA), /ENOENT/);
  assert.throws(() => inlineHtml(html, CSS, '/nonexistent/js.html', MOBILE, LIB_JSQR, LIB_QUAGGA, CAMERA), /ENOENT/);
  assert.throws(() => inlineHtml(html, CSS, JS, '/nonexistent/mobile.html', LIB_JSQR, LIB_QUAGGA, CAMERA), /ENOENT/);
  assert.throws(() => inlineHtml(html, CSS, JS, MOBILE, '/nonexistent/lib-jsqr.html', LIB_QUAGGA, CAMERA), /ENOENT/);
  assert.throws(() => inlineHtml(html, CSS, JS, MOBILE, LIB_JSQR, '/nonexistent/lib-quagga.html', CAMERA), /ENOENT/);
  assert.throws(() => inlineHtml(html, CSS, JS, MOBILE, LIB_JSQR, LIB_QUAGGA, '/nonexistent/camera-scan.html'), /ENOENT/);
});
...
test('index.html THẬT: bản inline tự-chứa hoàn chỉnh (css + mobile + libs + camera + js)', () => {
  ...
  for (const marker of ['--primary', '.task-pagin', 'STAFF-CACHE-START', 'function dashScrollTop', 'function submitScanMealMove', 'function openCameraScan', 'function onCameraDecoded', 'window.jsQR', 'window.Quagga']) {
