...
<p align="center">
  <img src="https://img.shields.io/badge/tests-488%20passing-188038?style=for-the-badge&logo=vitest&logoColor=white" alt="tests 488" />
...
`🎯 TL;DR` • `✨ Bento` • `🏗️ Kiến trúc` • `🧱 Tech` • `📁 Cấu trúc` • `🗄️ Sheets` • `⚡ 30s Start` • `🧪 488 Tests` • `🚀 Deploy` • `📏 Quy ước`
...
- **Ẩn Danh** (task Điểm Danh Ca): công tắc lúc tạo + nút Bật/Tắt trên topbar màn quét — bật = ẩn khỏi danh sách chung (email tạo task + email chủ script thấy mọi task ẩn ở mọi trạng thái trên mọi thiết bị + badge `Ẩn`; email khác giữ lọc theo thiết bị tạo, không mở được — kể cả sau **Hoàn Thành** vẫn ẩn); **Tắt** → hiện lại như thường
...
- Task **Đang Mở** nền **Aurora trôi** (xanh–lam–vàng, desktop + card mobile, tắt khi `prefers-reduced-motion`) + **thanh tiến độ Đã quét/Tổng** trong ô Trạng thái (desktop) và dưới thẻ (mobile)
...
| 🧪 **Test** | ![Node](https://img.shields.io/badge/Node-%3E%3D22-339933?style=flat-square&logo=nodedotjs&logoColor=white) ![Tests](https://img.shields.io/badge/tests-488_passing-188038?style=flat-square) | `node:test` 30f/389 + `unittest` 87 + Chrome 12 = **488** · `check:drift` SSOT guard |
...
├── tests/                 # 30 file, 389 tests — node:test
...
```bash
npm test            # 389 — node --test tests/*.test.js
...
<p align="center">
  <img src="https://img.shields.io/badge/Node-432%2F432-brightgreen?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Python-97%2F97-brightgreen?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Chrome-12%2F12-brightgreen?style=for-the-badge" />
  <img src="https://img.shields.io/badge/total-541%20passing-188038?style=for-the-badge" />
</p>
...
```bash
npm run build:local
npm test              # 432/432 pass — 34 files
npm run test:py       # 97/97 pass — 5 files api/test_*.py
npm run test:chrome   # 12/12 pass (khi đổi UI/scan/mock) — cần Node ≥22 + Chrome
```

| Lệnh | Chạy gì | Khi nào bắt buộc |
| :--- | :------ | :--------------- |
| `npm test` | 34 file, 432 tests `node:test` — ScanLogic/CsvUtil/TaskSearch + smoke `.gs` + camera/OCR/drift/task-hide | **Mọi commit** |
| `npm run test:py` | 97 tests `api/database.py`/`scanlogic.py`/`services.py` mirror GAS | Đổi `*.gs`/`api/*.py` |
...
- ✅ **Test 389 + 87 + 12 = 488 pass** · `check:drift` guard · CI `deploy-gas` concurrency
