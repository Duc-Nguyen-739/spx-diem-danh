...
- **Ẩn Danh** (task Điểm Danh Ca): công tắc lúc tạo + nút Bật/Tắt trên topbar màn quét — bật = ẩn khỏi danh sách chung (email tạo task (`createdBy`) + email chủ script (`getMeta.ownerEmail`) thấy mọi task ẩn ở mọi trạng thái trên mọi thiết bị + badge `Ẩn`; email khác giữ lọc theo thiết bị tạo, không mở được — kể cả sau **Hoàn Thành** vẫn ẩn); **Tắt** → hiện lại như thường (cột `isHidden` sheet AttendanceTask)
